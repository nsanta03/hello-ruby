...
complete_profile = { :name => { :first => "Brian", :last => "Eng"}, 
                     ...
                     :timeline => [
                       { :status => "Eating tacos", :posted => "7:30am"}, 
...
# Side note:
# So far, we have only used "symbols" as the keys to our hash, i.e. a word preceded by a colon.
# This is typical for a Hash written by you. However, a Hash key can be of any type. You will
# run into this later, when the Hash is coming from an external source, such as an API or database.
my_profile = { "name" => "Brian", "location" => "Chicago" }
puts my_profile[:name] # Nothing
puts my_profile["name"] # "Brian"